...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || 'Login failed. Try demo@zamota.com / password123');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDemo = async () => {
    setEmail('demo@zamota.com');
    setPassword('password123');
    setLoading(true);
    try {
      await login('demo@zamota.com', 'password123');
      toast.success('Logged in as Demo User 🎉');
      navigate('/');
    } catch {
      toast.error('Backend not running — exploring in demo mode');
      navigate('/');
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="absolute -bottom-40 -right-40 w-96 h-96 bg-brand-800/20 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="w-full max-w-md animate-slide-up">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-white/40 text-sm">Sign in to continue ordering</p>
        </div>
        ...
        <div className="glass rounded-2xl p-8 shadow-2xl">
          ...
          <button
            onClick={handleDemo}
            className="w-full flex items-center justify-center gap-3 btn-ghost mb-6 group"
          >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end">
              <button type="button" className="text-brand-400 text-sm hover:text-brand-300 transition-colors">
                Forgot password?
              </button>
            </div>
